...
- Изменив значение переменной REQPASS на отличное от **nopass**, получим запрос ввода пароля для клиентского ключа. Иногда полезно, когда *клиент* будет запрашивать пароль от ключа, при попытке подключиться к серверу.
...
```
systemctl enable openvpn@srv1
systemctl start openvpn@srv1
```
